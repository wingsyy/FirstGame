...
public class BOSSBullet : MonoBehaviour
{
    ...
    private void Start()
    {
        HPCount = GetComponent<BOSSHP>();
    }
    public void Update()
    {

        //如果敌人存活，发射子弹
        if (timer <= Time.time && HPCount.CurrentHP > 0 && (this.transform.position - Player.transform.position).magnitude < 15)
        {

            GenerateAmmo();//生成子弹
            timer = Time.time + Random.Range(3,5);//间隔三到五秒
        }
    }
    public void GenerateAmmo()
    {
        ...
        if (eb != null)
        {
            eb.Move((Player.transform.position - this.transform.position).normalized, 300);
        }
    }
}
